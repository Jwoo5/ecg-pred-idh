...
class Wav2Vec2Model(ConvTransformerModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.same_quantizer = same_quantizer
        ...
        if quantize_targets:
            vq_dim = latent_dim if latent_dim > 0 else self.final_dim
            self.quantizer = GumbelVectorQuantizer(
                dim=self.embed,
                num_vars=latent_vars,
                temp=latent_temp,
                groups=latent_groups,
                combuine_groups=False,
                vq_dim=vq_dim,
                time_first=True,
            )
        ...
        self.final_proj = nn.Linear(self.encoder_embed_dim, self.final_dim)
